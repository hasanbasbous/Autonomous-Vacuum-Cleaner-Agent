...
public class TwoAgents {
    ...
    private final int rectDimensions;
    private int speed;


    public TwoAgents(GridPane gridPane, int dimensions) {
        this.gridPane = gridPane;
        rectDimensions = dimensions;
    }

    // the method the vacuum cleaner will implement
    public void shortestPathDirt() throws InterruptedException, IOException, ClassNotFoundException {
        ...
        if (Main.vacuumClnrCellList.size() > 0) {
            int[] temp = Main.vacuumClnrCellList.remove();
            while (Main.vacuumClnrCellList.size() > 0 && Main.grid[temp[0]][temp[1]] != 1) {
                temp = Main.vacuumClnrCellList.remove();
            }
            if (Main.grid[temp[0]][temp[1]] == 1)
                path = shortestPathDirty_1(Main.startPosVacClnr, temp).path;
        } else {
        ...
        if (path.size() != 0) {
            Main.vacuumClnrCellListNew.add(new int[]{path.getLast().x, path.getLast().y});
            System.out.println("VacClnr: " + path);
            int x = Main.startPosVacClnr[0], y = Main.startPosVacClnr[1];
...
//            Main.flag = 1;
        ...
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++) {
                if (Main.grid[i][j] != 0 && Main.grid[i][j] != 3)
...
//            System.out.println("VacClnr: " + path);
    ...
    public void shortestPathClean() throws InterruptedException {
        ...
        if (Main.dirtPrdcrCellList.size() > 0) {
            int[] temp = Main.dirtPrdcrCellList.remove();
            while (Main.dirtPrdcrCellList.size() > 0 && Main.grid[temp[0]][temp[1]] != 2) {
                temp = Main.dirtPrdcrCellList.remove();
            }
            if (Main.grid[temp[0]][temp[1]] == 2)
                path = shortestPathClean_1(Main.startPosDrtPrdcr, temp).path;
        } else {
        ...
        if (path.size() != 0) {
            Main.dirtPrdcrCellListNew.add(new int[]{path.getLast().x, path.getLast().y});
            System.out.println("DrtPrdcr: " + path);
...
//            Main.text.setText("flag: " + String.format("%.2f", Main.flag));
//            Thread.sleep(1);
////            x = Main.startPosDrtPrdcr[0];
////            y = Main.startPosDrtPrdcr[1];
////            Main.dirtyTiles++;
...
//            Main.flag = -1;
        ...
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++) {
                if (Main.grid[i][j] != 0 && Main.grid[i][j] != 4)
                    cells[i][j] = new Cell(i, j, Integer.MAX_VALUE, null);
            }
    ...
    public void shortestPathDisplayVacClnr(LinkedList<Cell> path) throws InterruptedException {
        ...
        if (path.size() != 0) {
            ...
            if (Main.grid[end[0]][end[1]] != 3) {
                ...
                if (Main.grid[end[0]][end[1]] == 1) {
                    Main.cleanTiles++; // add it to the cleaned tiles by the vacuum cleaner
                    Main.grid[end[0]][end[1]] = 2;
                    //remove from dirt
                    for (int i = 0; i < Main.dirt.size(); i++) {
                        if (end[0] == Main.dirt.get(i)[0] && end[1] == Main.dirt.get(i)[1]) {
                            Main.dirt.remove(i);
                            break;
                        }
                    }
...
//            }
    ...
    public void shortestPathDisplayDirtPrdcr(LinkedList<Cell> path) throws InterruptedException {
        ...
        if (path.size() != 0) {
            ...
            if (Main.grid[c1.x][c1.y] != 4) {
                if (Main.grid[end[0]][end[1]] == 2) {
                    Main.dirtyTiles++;
                    Main.grid[end[0]][end[1]] = 1;
                    for (int i = 0; i < Main.clean.size(); i++) {
                        if (end[0] == Main.clean.get(i)[0] && end[1] == Main.clean.get(i)[1]) {
                            Main.clean.remove(i);
                            break;
                        }
                    }
